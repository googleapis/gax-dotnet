...
namespace Google.Api.Gax.Grpc.Gcp
{
    ...
    public class GcpCallInvoker : CallInvoker
    {
        ...
        public GcpCallInvoker(string target, ChannelCredentials credentials, GrpcChannelOptions options, ApiConfig apiConfig, GrpcAdapter adapter)
        {
            ...
            _apiConfig = apiConfig.Clone();
            _channelOptions = GaxPreconditions.CheckNotNull(options, nameof(options));

            GaxPreconditions.CheckArgument(_apiConfig.ChannelPool is object, nameof(apiConfig), "Invalid API config: no channel pool settings");
            _affinityByMethod = InitAffinityByMethodIndex(_apiConfig);
        }
        ...
        private ChannelRef GetChannelRef(string affinityKey = null)
        {
            ...
            lock (_thisLock)
            {
                ...
                if (count < _apiConfig.ChannelPool.MaxSize)
                {
                    ...
                    ChannelBase channel = _adapter.CreateChannel(_target, _credentials, _channelOptions.WithCustomOption(ClientChannelId, Interlocked.Increment(ref clientChannelIdCounter)));
