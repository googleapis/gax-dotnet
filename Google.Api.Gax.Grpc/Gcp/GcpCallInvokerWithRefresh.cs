...
namespace Google.Api.Gax.Grpc.Gcp
{
    ...
    /// </summary>
    public sealed class GcpCallInvokerWithRefresh : CallInvoker
    {
        ...
        private readonly IDictionary<Guid, string> _reverseMapChannelRefByAffinityKey = new Dictionary<Guid, string>();
        private readonly ConcurrentQueue<ChannelRef> _channelRefs = new ConcurrentQueue<ChannelRef>();
        ...
        private Action<CallInvoker> _channelPrimer;
        private TimeSpan _maintenanceLoopDelay;
        private TimeSpan _timeout;
        private int _channelsInGrace = 0;
        ...
        public GcpCallInvokerWithRefresh(ServiceMetadata serviceMetadata, string target, ChannelCredentials credentials, GrpcChannelOptions options, ApiConfig apiConfig, GrpcAdapter adapter, Action<CallInvoker> channelPrimer, TimeSpan maintenanceDelay, TimeSpan timeout)
        {
            ...
            _channelPrimer = channelPrimer;
            _maintenanceLoopDelay = maintenanceDelay;
            _timeout = timeout;
            if(_maintenanceLoopDelay != TimeSpan.Zero)
            {
                Task.Run(() => ManageChannels());
            }   
        }

        private async Task ManageChannels()
        {

            while (true)
            {
                //Console.WriteLine($"{DateTime.UtcNow:HH:mm:ss.fff} number of channels {_channelRefs.Count} channels in grace {_channelsInGrace}");
                foreach (var channel in _channelRefs)
                {
                    //Console.WriteLine($"{DateTime.UtcNow:HH:mm:ss.fff} number of streams in channel {channel._channelId} is {channel.ActiveStreamCount}");
                    var timeTillNow = DateTime.UtcNow - channel.ChannelStartTime;
                    if (timeTillNow >= _timeout)
                    {
                        Console.WriteLine($"{DateTime.UtcNow:HH:mm:ss.fff} creating new channel");
                        ChannelBase newChannel = _adapter.CreateChannel(_serviceMetadata, _target, _credentials, _channelOptions.WithCustomOption(ClientChannelId, Interlocked.Increment(ref s_clientChannelIdCounter)));
                        ChannelRef channelRef = new ChannelRef(newChannel, 0 , channelPrimer: _channelPrimer, channelId: Guid.NewGuid());
                        _channelRefs.Enqueue(channelRef);
                        Interlocked.Increment(ref _channelsInGrace);
                        channel.Grace = true;
                        _reverseMapChannelRefByAffinityKey.TryGetValue(channel._channelId, out var methodAffinityWithOldChannel);
                        if(methodAffinityWithOldChannel != null)
                        {
                            _channelRefByAffinityKey.Remove(methodAffinityWithOldChannel);
                        }
                        _reverseMapChannelRefByAffinityKey.Remove(channel._channelId);
                    }
                }
                await RemoveInactiveChannelsAsync().ConfigureAwait(false);
                await Task.Delay(_maintenanceLoopDelay).ConfigureAwait(false);
            }
        }

        private async Task RemoveInactiveChannelsAsync()
        {
            while(_channelRefs.Count > 0)
            {
                var currChannel = _channelRefs.First();
                if(currChannel.Grace == true && currChannel.ActiveStreamCount == 0)
                {
                    _channelRefs.TryDequeue(out var oldChannel);
                    Interlocked.Decrement(ref _channelsInGrace);
                    Console.WriteLine($"{DateTime.UtcNow:HH:mm:ss.fff} channel {oldChannel._channelId} has been removed.");
                    await oldChannel.Channel.ShutdownAsync().ConfigureAwait(false);
                }
                else
                {
                    break;
        ...
        private ChannelRef GetChannelRef(string affinityKey = null)
        {
            ...
            lock (_thisLock)
            {
                ...
                foreach (ChannelRef channelRef in orderedChannelRefs)
                {
                    ...
                    {
                        if (channelRef.Grace == false)
                        {
                            return channelRef;
                        }
                        else if (channelRef.Grace == true)
                        {
                            continue;
                        }
                    }
                    ...
                    {
                        // useful when all the channels are running at its full capacity.
                        if (channelRef.Grace == false)
                        {
                            tempChannel = channelRef;
                        }
                    }
                }
                ...
                if (count < _apiConfig.ChannelPool.MaxSize)
                {
                    ...
                    ChannelRef channelRef = new ChannelRef(channel, count, channelPrimer: _channelPrimer, channelId: Guid.NewGuid());
                    _channelRefs.Enqueue(channelRef);
                    return channelRef;
                }
                if (tempChannel == null)
                {
                    throw new Exception(" Something wrong with channel pool implementation");
                }
            ...
            if (!string.IsNullOrEmpty(affinityKey))
            {
                lock (_thisLock)
                {
                    ...
                    if (!_channelRefByAffinityKey.Keys.Contains(affinityKey))
                    {
                        ...
                        _reverseMapChannelRefByAffinityKey.Add(channelRef._channelId, affinityKey);
                    }
                ...
                lock (_thisLock)
                {
                    if (_channelRefByAffinityKey.TryGetValue(affinityKey, out ChannelRef channelRef))
                    {
                        ...
                        if (newCount <= 0)
                        {
                            ...
                            _reverseMapChannelRefByAffinityKey.Remove(channelRef._channelId);
                        }
                    }
                }
            }
        ...
        public async Task ShutdownAsync()
        {
            while(_channelRefs.Count>0)
            {
                _channelRefs.TryDequeue(out var channel);
                await channel.Channel.ShutdownAsync().ConfigureAwait(false);
            }
        }
