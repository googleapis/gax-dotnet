...
internal sealed partial class HttpRuleTranscoder
{
    ...
    private class QueryParameterField
    {
        ...
        internal IEnumerable<string> GetValues(IMessage request)
        {
            ...
            if (!_includeDefaultValues && ProtobufUtilities.IsDefaultValue(value))
            {
                yield break;
            }
            ...
            if (_field.IsRepeated && value is IList list)
            {
                ...
                {
                    yield return ProtobufUtilities.FormatValueAsJsonPrimitive(item);
                }
            }
            else
            {
                yield return ProtobufUtilities.FormatValueAsJsonPrimitive(value);
            }
        }
    }
}
