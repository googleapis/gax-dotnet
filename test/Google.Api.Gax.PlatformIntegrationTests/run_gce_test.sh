...
cp app.gce.yaml ./publish/app.yaml
cd publish
echo "Deploying to GCE. This can take several minutes..."
...
url="https://test-dot-platformtest-gce-canbedeleted-dot-$projectId.appspot.com"
echo "Retrieving test web page: $url"
testResponse=$(curl $url | head -n1)
echo "Test response: $testResponse"

echo "Deleting GCE service. This can take a few minutes..."
gcloud.cmd app services delete platformtest-gce-canbedeleted --quiet --project=$projectId
if [ $? -ne 0 ]; then
    echo "ERROR: gcloud app services delete failed";
    exit 1
fi

if [ "$testResponse" != "Gce" ]; then
