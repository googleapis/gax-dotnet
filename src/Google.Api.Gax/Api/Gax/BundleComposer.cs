...
namespace Google.Api.Gax
{
        ...
        where TResponse : class, IMessage<TResponse>
    {
        ...
        {
            private readonly Func<TRequest, RepeatedField<TRequestEntry>> requestEntryExtractor;
            private readonly Func<TResponse, RepeatedField<TResponseEntry>> responseEntryExtractor;
            ...
            {
                this.requestEntryExtractor = GaxPreconditions.CheckNotNull(requestEntryExtractor, nameof(requestEntryExtractor));
                this.responseEntryExtractor = GaxPreconditions.CheckNotNull(responseEntryExtractor, nameof(responseEntryExtractor));
            }

            internal override IList<TResponse> SplitResponse(IList<TRequest> requests, TResponse response)
            {
                var totalRequestEntries = requests.Sum(r => requestEntryExtractor(r).Count);
                ...
                var responseEntries = responseEntryExtractor(response);
                ...
                responseEntryExtractor(splitResponseTemplate).Clear();

                int responseEntryIndex = 0;
                foreach (var request in requests)
                {
                    var requestEntryCount = requestEntryExtractor(request).Count;
                    var individualResponse = splitResponseTemplate.Clone();
                    var individualEntries = responseEntryExtractor(individualResponse);
            ...
            {
                var existingEntries = requestEntryExtractor(bundleRequest);
                var newEntries = requestEntryExtractor(extra);
                existingEntries.Add(newEntries);
            }

            internal override int GetEntryCount(TRequest request)
            {
                return requestEntryExtractor(request).Count;
            }

            internal override void ClearEntries(TRequest request)
            {
                requestEntryExtractor(request).Clear();
            }
        }
        ...
        {
            private readonly Func<TRequest, TKey> keySelector;
