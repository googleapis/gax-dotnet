...
namespace Google.Api.Gax
{
            ...
            where TResponse : class, IMessage<TResponse>
    {
        ...
        private readonly Dictionary<TRequest, Bundle> _waitingBundles;
        // These are bundles which are in flight - we may want to wait for them all to complete,
        // or cancel them all, etc.
        private readonly LinkedList<Bundle> _inFlightBundles;
            ...
            ApiCallable<TRequest, TResponse> callable)
        {
            ...
            _waitingBundles = new Dictionary<TRequest, Bundle>(composer.RequestComparer);
            _inFlightBundles = new LinkedList<Bundle>();
        }
        ...
        public Task<TResponse> Call(TRequest request, CancellationToken cancellationToken)
        {
            ...
            lock (_monitor)
            {
                ...
                newBundle = !_waitingBundles.TryGetValue(request, out bundle);
                if (newBundle)
                {
                    bundle = new Bundle(clientRequest);
                    _waitingBundles[request] = bundle;
                }
                else
                {
                ...
                // We don't absolutely need the null checks, but I'd prefer not to do the work
                // of getting the entry count and request size if we're just going to throw them away.
                sendImmediately =
            ...
            return () =>
            {
                ...
                lock (_monitor)
                {
                    ...
                    if (bundle.ClientCalls.Count == 0)
                    {
                        ...
                        _waitingBundles.Remove(bundle.CombinedRequest);
                    }
                    else
                    {
        ...
        private void TrySendBundle(Bundle bundle)
        {
            ...
            lock (_monitor)
            {
                ...
                _waitingBundles.Remove(bundle.CombinedRequest);
                _inFlightBundles.AddLast(bundle);
        ...
        private void HandleResponse(Task<TResponse> task, object bundleObject)
        {
            ...
            lock (_monitor)
            {
                ...
                _inFlightBundles.Remove(bundle);
